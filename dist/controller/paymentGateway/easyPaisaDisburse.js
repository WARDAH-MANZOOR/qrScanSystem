...
const getDisburseAccount = async (req, res, next) => {
    try {
        const result = await easyPaisaDisburse.getDisburseAccount(req.params.accountId);
        res.status(200).json(ApiResponse.success(result));
    }
    catch (error) {
        next(error);
...
const updateDisburseAccount = async (req, res, next) => {
    try {
        const result = await easyPaisaDisburse.updateDisburseAccount(req.params.accountId, req.body);
        res.status(200).json(ApiResponse.success(result));
    }
    catch (error) {
        next(error);
...
const deleteDisburseAccount = async (req, res, next) => {
    try {
        const result = await easyPaisaDisburse.deleteDisburseAccount(req.params.accountId);
        res.status(200).json(ApiResponse.success(result));
    }
    catch (error) {
        next(error);
