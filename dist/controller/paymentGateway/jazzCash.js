...
const initiateJazzCashClone = async (req, res, next) => {
    try {
        const paymentData = req.body;
        console.log("Payment Data: ", paymentData);
        const errors = validationResult(req);
        ...
        const result = await jazzCashService.initiateJazzCashPaymentClone(paymentData, merchantId);
        if (result.statusCode != "000") {
            res.status(result.statusCode != 500 ? result.statusCode : 201).send(ApiResponse.error(result, result.statusCode != 500 ? result.statusCode : 201));
            return;
        }
...
const initiateSandboxJazzCash = async (req, res, next) => {
    try {
        const paymentData = req.body;
        console.log("Payment Data: ", paymentData);
        const errors = validationResult(req);
        ...
        const result = await jazzCashService.initiateSandboxJazzCashPayment(paymentData, merchantId);
        if (result.statusCode != "000") {
            res.status(result.statusCode != 500 ? result.statusCode : 201).send(ApiResponse.error(result, result.statusCode != 500 ? result.statusCode : 201));
            return;
        }
...
const initiateProductionDisbursmentClone = async (req, res, next) => {
    try {
        console.log("IBFT Called");
        process.env['NODE_TLS_REJECT_UNAUTHORIZED'] = '0';
        const token = await getToken(req.params.merchantId);
...
const initiateProductionMWDisbursementClone = async (req, res, next) => {
    try {
        process.env['NODE_TLS_REJECT_UNAUTHORIZED'] = '0';
        const token = await getToken(req.params.merchantId);
        if (!token?.access_token) {
