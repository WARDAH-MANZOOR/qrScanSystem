...
import { comparePasswords, findUserByEmail, generateToken, getUserByEmail, getUserPassword, hashPassword, setTokenCookie, updateUserPassword, } from "../../services/authentication/index.js";
...
const createDecryptionKey = async (req, res, next) => {
    try {
        const id = parseInt(req.params.id);
        if (!id) {
            throw new CustomError("Invalid user id", 400);
...
const updatePassword = async (req, res, next) => {
    try {
        const old_password = await getUserPassword(req.user?.merchant_id);
        if (!(await comparePasswords(req.body.old_password, old_password?.password))) {
            throw new CustomError("Old password do not match", 500);
        }
        let new_password = await hashPassword(req.body.new_password);
        updateUserPassword(req.user?.merchant_id, new_password);
