...
const createPaymentRequestWithOtpClone = async (req, res, next) => {
    try {
        const result = await paymentRequestService.createPaymentRequestWithOtpClone(req.body, req.params.merchantId);
        if (result.status == 400) {
            throw new CustomError(result.message, result.status);
        }
        res.status(200).json(ApiResponse.success(result));
...
const payRequestedPaymentForRedirection = async (req, res, next) => {
    try {
        if (!req.body.payId) {
            throw new CustomError("Payment request ID is required", 400);
        }
...
const preRequest = async (req, res) => {
    ...
    if (!accountNo || !location)
        return res.status(422).json({ error: "phone_and_location_required" });
    if (!validateLocation(location?.lat, location?.lng))
        return res.status(422).json({ error: "invalid_location" });
