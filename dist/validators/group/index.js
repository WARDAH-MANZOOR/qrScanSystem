...
const validateUpdateGroup = [
    ...
    body("permissionIds.remove")
        ...
        if (value.length > 0) {
            ...
            const missingPermissions = value.filter((id) => !existingPermissionIds.includes(id));
            if (missingPermissions.length > 0) {
                throw new Error(`Permissions ${missingPermissions.join(", ")} are not assigned to this group`);
            }
        }
    }),
    // validation
