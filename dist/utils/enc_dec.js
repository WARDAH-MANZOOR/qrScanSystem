...
const key = Buffer.from(process.env.ENCRYPTION_KEY, 'hex'); // 32 bytes key for AES-256
...
function encryptData(payload, secretKey, iv) {
    // Ensure the payload is a JSON string
    const jsonString = JSON.stringify(payload);
    // Create a cipher instance
    const cipher = crypto.createCipheriv('aes-128-cbc', Buffer.from(secretKey), new Uint8Array(Buffer.from(iv)));
    // Encrypt the data
...
function decryptData(encryptedData, secretKey, iv) {
    ...
    const decipher = crypto.createDecipheriv('aes-128-cbc', Buffer.from(secretKey), new Uint8Array(Buffer.from(iv)));
    // Decrypt the data
...
async function callbackEncrypt(payload, userId) {
    try {
        console.log("User Id: ", userId);
        const key = await authenticationService.getDecryptionKey(userId);
        console.log("Key: ", key);
        const cipher = crypto.createCipheriv('aes-256-gcm', Buffer.from(key, 'hex'), new Uint8Array(iv));
...
async function callbackDecrypt(encryptedData, iv, tag) {
    try {
        ...
        // Create decipher
        const decipher = crypto.createDecipheriv('aes-256-gcm', Buffer.from(key, "hex"), new Uint8Array(ivBuffer));
