                ...
                try {
                    ...
                    if (txn.provider?.toUpperCase() === "EASYPAISA") {
                        ...
                        if (txn.to_provider.toUpperCase() === "JAZZCASH") {
                            ...
                            await updateTransactionClone(token?.access_token, txn, merchant?.uid);
                        }
                    }
                    ...
                    if (error?.message && error.message.includes("Not Enough Balance")) {
                        console.log(`Error processing transaction ${txn.system_order_id}: ${error.message}. Skipping all transactions for merchant ${merchantId}.`);
                        skipMerchant = true;
                        break; // break out of the transactions loop for this merchant
                    }
                    else {
                        // For other errors, log and continue with the next transaction.
                        console.log(`Error processing transaction ${txn.system_order_id}: ${error.message}. Continuing with next transaction.`);
                    }
                }
            }
