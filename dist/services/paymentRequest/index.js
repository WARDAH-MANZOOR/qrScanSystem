...
const createPaymentRequest = async (data, user) => {
    try {
...
const createPaymentRequestWithOtp = async (data, user) => {
    try {
        if (!user) {
            throw new CustomError("User Id not given", 404);
        }
...
const createPaymentRequestWithOtpClone = async (data, user) => {
    try {
        if (!user) {
            throw new CustomError("User Id not given", 404);
        }
...
const payRequestedPayment = async (paymentRequestObj) => {
    try {
        ...
        if (paymentRequestObj.provider?.toLocaleLowerCase() === "jazzcash") {
            ...
            if (jazzCashPayment.statusCode != "000") {
                ...
                throw new CustomError(jazzCashPayment.message, 500);
            }
        }
        else if (paymentRequestObj.provider?.toLocaleLowerCase() === "easypaisa") {
            if (merchant.easypaisaPaymentMethod === "DIRECT") {
                ...
                if (paymentRequest.merchant_transaction_id == null && !paymentRequest.transactionId) {
                    easyPaisaPayment = await easyPaisaService.initiateEasyPaisa(merchant.uid, {
                    ...
                    easyPaisaPayment = await easyPaisaService.initiateEasyPaisaForRedirection(merchant.uid, {
                        order_id: paymentRequest.merchant_transaction_id || paymentRequest.transactionId,
                        amount: paymentRequest.amount,
                        type: "wallet",
                        phone: paymentRequestObj.accountNo || paymentRequest?.metadata?.phone,
                ...
                if (easyPaisaPayment.statusCode != "0000") {
                    throw new CustomError(easyPaisaPayment.message, 500);
                }
            }
                ...
                if (swichPayment?.statusCode != "0000") {
                    throw new CustomError(swichPayment.message, 500);
                }
            }
            else {
        ...
        else if (paymentRequestObj.provider?.toLocaleLowerCase() === "card") {
            let commission;
            if (+merchant?.commissions[0].cardRate != 0) {
                commission = +merchant?.commissions[0].commissionGST +
                    +merchant?.commissions[0].cardRate +
...
const payRequestedPaymentForRedirection = async (paymentRequestObj) => {
    try {
        const paymentRequest = await prisma.paymentRequest.findFirst({
            where: {
                id: paymentRequestObj.payId,
        ...
        console.log(merchant?.easypaisaPaymentMethod);
        let response;
        if (paymentRequestObj.provider?.toLocaleLowerCase() === "easypaisa") {
            if (merchant.easypaisaPaymentMethod === "DIRECT") {
                // easypaisa payment
                response = await easyPaisaService.initiateEasyPaisaForRedirection(merchant.uid, {
                    order_id: paymentRequest.merchant_transaction_id,
                    amount: paymentRequest.amount,
                    type: "wallet",
                    phone: paymentRequestObj.accountNo || paymentRequest?.metadata?.phone,
                ...
                if (response.statusCode != "0000") {
                    throw new CustomError(response.message, 500);
                }
            }
            else {
                const token = await payfast.getApiToken(merchant.uid, {});
...
const payUpaisaZindigi = async (paymentRequestObj) => {
    try {
        const paymentRequest = await prisma.paymentRequest.findFirst({
            where: {
                id: paymentRequestObj.payId,
...
const getPaymentRequestbyId = async (paymentRequestId) => {
    try {
        const paymentRequest = await prisma.paymentRequest.findFirst({
            where: {
                id: paymentRequestId,
        ...
        throw new CustomError(error?.message ||
            "An error occurred while retrieving the payment request", error?.statusCode || 500);
    }
};
const getPaymentRequest = async (obj) => {
    try {
...
const updatePaymentRequest = async (paymentRequestId, data, user) => {
    try {
        ...
        if (!updatedPaymentRequest) {
            throw new CustomError("An error occurred while updating the payment request", 500);
        }
        return {
...
const deletePaymentRequest = async (paymentRequestId) => {
    try {
        ...
        if (!deletedPaymentRequest) {
            throw new CustomError("An error occurred while deleting the payment request", 500);
        }
        return {
        ...
        throw new CustomError(error?.message || "An error occurred while deleting the payment request", error?.statusCode || 500);
    }
};
export default {
