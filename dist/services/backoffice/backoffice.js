...
async function removeMerchantFinanceData(merchantId) {
    try {
...
async function zeroMerchantWalletBalance(merchantId) {
    try {
...
async function checkMerchantTransactionStats(merchantId) {
    try {
...
async function settleDisbursements(transactionIds) {
    try {
...
async function failDisbursementsWithAccountInvalid(transactionIds) {
    try {
        const txns = await prisma.disbursement.findMany({
            where: {
                merchant_custom_order_id: { in: transactionIds },
...
async function settleAllMerchantTransactionsUpdated(merchantId) {
    try {
...
const createTransactionService = async (body, merchant_id) => {
    try {
        const merchant = await prisma.merchantFinancialTerms.findUnique({
    ...
    for (const merchantId in transactionsByMerchant) {
        ...
        for (const txn of merchantTxns) {
            ...
            if (merchant?.callback_mode == "DOUBLE") {
                webhook_url = merchant?.payout_callback;
            }
            else {
                webhook_url = merchant?.webhook_url;
            }
            await transactionService.sendCallback(webhook_url, { original_amount: txn.transactionAmount, date_time: txn.disbursementDate, merchant_transaction_id: txn.merchant_custom_order_id, merchant_id: txn.merchant_id }, txn?.account, "payout", merchant?.encrypted == "True" ? true : false, false);
        }
    }
}
...
async function processTodaySettlements() {
    ...
    for (const report of settlementReports) {
        try {
            ...
            console.log(`Updating merchant ${merchant.merchant_id}: Deducting ${deduction} from available balance and adding it to disbursement balance.`);
            // 4. Update the merchant record.
            //    We use atomic operations 'decrement' and 'increment' to update the balances.
            const { walletBalance } = await getWalletBalance(merchant.merchant_id);
...
async function adjustMerchantDisbursementBalance(merchantId, targetBalance, record, type) {
    try {
        // Get current balance
        let walletBalance;
        // if (!wb) {
