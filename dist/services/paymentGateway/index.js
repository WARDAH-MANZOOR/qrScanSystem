...
async function simpleSandboxGetToken(merchantId) {
    try {
        // validate Merchant
        const findMerchant = await merchantService.findOne({
            uid: merchantId,
...
async function simpleGetToken(merchantId) {
    try {
        const myHeaders = new Headers();
...
//   }
// }
async function initiateTransaction(token, body, merchantId) {
    let findMerchant;
    let walletBalance;
...
async function initiateTransactionClone(token, body, merchantId) {
    let findMerchant;
    let walletBalance;
    let totalDisbursed = new Decimal(0);
    let balanceDeducted = false;
...
async function simpleSandboxinitiateTransactionClone(token, body, merchantId) {
    let findMerchant;
    let walletBalance;
    let totalDisbursed = new Decimal(0);
    let obj = {};
...
async function simpleProductionInitiateTransactionClone(token, body, merchantId) {
    let findMerchant;
    let walletBalance;
    let totalDisbursed = new Decimal(0);
    let obj = {};
...
async function updateTransaction(token, body, merchantId) {
    let findMerchant;
    let walletBalance;
    let totalDisbursed = new Decimal(0);
    try {
...
async function updateTransactionClone(token, body, merchantId) {
    let findMerchant;
    let walletBalance;
    let totalDisbursed = new Decimal(0);
    try {
...
//   }
// }
async function mwTransaction(token, body, merchantId) {
    let findMerchant;
    let walletBalance;
    let totalDisbursed = new Decimal(0);
    let balanceDeducted = false;
...
async function mwTransactionClone(token, body, merchantId) {
    let findMerchant;
    let walletBalance;
    let totalDisbursed = new Decimal(0);
    try {
        ...
        let totalWithholdingTax = new Decimal(0);
        let amountDecimal = new Decimal(body.amount);
        let merchantAmount = new Decimal(body.amount);
        let data = {};
        let id = transactionService.createTransactionId();
        if (body.order_id) {
            data["merchant_custom_order_id"] = body.order_id;
...
async function simpleProductionMwTransactionClone(token, body, merchantId) {
    let findMerchant;
    let walletBalance;
    let totalDisbursed = new Decimal(0);
    let obj = {};
...
async function simpleSandboxMwTransactionClone(token, body, merchantId) {
    let findMerchant;
    let walletBalance;
    let totalDisbursed = new Decimal(0);
    let obj = {};
...
async function updateMwTransaction(token, body, merchantId) {
    let findMerchant;
    let walletBalance;
    let totalDisbursed = new Decimal(0);
    try {
...
async function checkTransactionStatus(token, body, merchantId) {
    // validate Merchant
    const findMerchant = await merchantService.findOne({
        uid: merchantId,
    });
    ...
    for (const id of body.transactionIds) {
        ...
        const payload = encryptData({ originalReferenceId: transaction.system_order_id, referenceID: transactionService.createTransactionId() }, findDisbureMerch.key, findDisbureMerch.initialVector);
        const requestData = {
            data: payload
        };
...
async function simpleCheckTransactionStatus(token, body, merchantId) {
    const findMerchant = await merchantService.findOne({
        uid: merchantId,
    });
    if (!findMerchant) {
    ...
    const payload = encryptData({ ...body, originalReferenceId: transaction?.system_order_id }, findDisbureMerch.key, findDisbureMerch.initialVector);
    const requestData = {
        data: payload
    };
...
async function simpleSandboxCheckTransactionStatus(token, body, merchantId) {
    ...
    for (const id of body.transactionIds) {
        ...
        const payload = encryptData({ originalReferenceId: id, referenceID: transactionService.createTransactionId() }, findDisbureMerch.key, findDisbureMerch.initialVector);
        const requestData = {
            data: payload
        };
        console.log(requestData);
