...
import { Prisma, ProviderEnum } from "@prisma/client";
import fs from "fs";
...
const initiateEasyPaisaForRedirection = async (merchantId, params) => {
    let saveTxn;
    let id = transactionService.createTransactionId();
    let reservations = [];
    try {
...
const createDisbursement = async (obj, merchantId) => {
    try {
        ...
        const ma2ma = await axios
            .post(`${process.env.EASYPAISA_URL}/epd-ma`, {
...
export const exportDisbursement = async (merchantId, params) => {
    try {
        const startDate = params?.start?.replace(" ", "+");
        const endDate = params?.end?.replace(" ", "+");
        ...
        while (hasMore) {
            const batch = await prisma.disbursement.findMany({
                where: filters,
