...
const addDisburseAccount = async (payload) => {
    try {
        ...
        throw new CustomError(error?.message || "An error occurred while creating the disburse account", 500);
    }
};
const getDisburseAccount = async (accountId) => {
    try {
        const where = {
            deletedAt: null,
        };
        ...
        throw new CustomError(error?.message ||
            "An error occurred while retrieving the disburse account", 500);
    }
};
const updateDisburseAccount = async (accountId, payload) => {
    try {
        ...
        throw new CustomError(error?.message || "An error occurred while updating the disburse account", 500);
    }
};
const deleteDisburseAccount = async (accountId) => {
    try {
        ...
        throw new CustomError(error?.message || "An error occurred while deleting the disburse account", 500);
    }
};
export default {
    addDisburseAccount,
