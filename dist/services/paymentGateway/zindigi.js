...
const walletToWalletPayment = async (body, clientInfo) => {
    try {
        let id = generateUniqueSixDigitNumber();
        let date = transactionService.createTransactionId().slice(1, 15);
        const response = await axios.post('https://z-sandbox.jsbl.com/zconnect/api/v2/w2wp-blb', {
            'w2wpRequest': {
                "MerchantType": "0088",
                "TraceNo": id,
                "CompanyName": "NOVA",
...
const debitInquiry = async (body) => {
    try {
        const response = await axios.post('https://z-sandbox.jsbl.com/zconnect/api/v2/debitinquiry2-blb2', {
...
const debitPayment = async (body, initiateResponse) => {
    try {
        const response = await axios.post('https://z-sandbox.jsbl.com/zconnect/api/v2/debitpayment-blb2', {
...
const transactionInquiry = async (body) => {
    try {
...
const getMerchant = async (merchantId) => {
    try {
        ...
        merchant = merchant.map((obj) => {
            ...
            obj["clientSecret"] = decrypt(obj["clientSecret"]);
            obj["organizationId"] = decrypt(obj["organizationId"]);
            return obj;
        });
        if (!merchant) {
        ...
        const updatedMerchant = await prisma.$transaction(async (tx) => {
            return tx.zindigiMerchant.update({
                ...
                data: {
                    ...
                    clientSecret: updateData.clientSecret != undefined
                        ? encrypt(updateData.clientSecret)
                        : existingMerchant.clientSecret,
                    organizationId: updateData.organizationId != undefined
