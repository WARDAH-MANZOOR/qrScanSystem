...
import { callbackDecrypt } from "../../utils/enc_dec.js";
...
function calculateHmacSha256(json, integrityText) {
    try {
...
const initiateJazzCashPaymentForRedirection = async (paymentData, merchant_uid) => {
    let refNo = "";
    try {
        console.log(JSON.stringify({ event: "JASSCASH_PAYIN_INITIATED", orderId: paymentData.order_id, body: paymentData }));
        var JAZZ_CASH_MERCHANT_ID = null;
...
const initiateJazzCashPaymentClone = async (paymentData, merchant_uid) => {
    let refNo = "";
    try {
        console.log(JSON.stringify({ event: "JASSCASH_PAYIN_INITIATED", orderId: paymentData.order_id, body: paymentData }));
        var JAZZ_CASH_MERCHANT_ID = null;
        ...
        if (paymentType === "CARD") {
            ...
            // Send the request to JazzCash
            const paymentUrl = "https://payments.jazzcash.com.pk/ApplicationAPI/API/Payment/DoTransaction";
            const headers = {
                "Content-Type": "application/x-www-form-urlencoded",
            };
            const response = await axios.post(paymentUrl, new URLSearchParams(sendData).toString(), { headers });
...
const initiateSandboxJazzCashPayment = async (paymentData, merchant_uid) => {
    let refNo = "";
    try {
        console.log(JSON.stringify({ event: "JASSCASH_PAYIN_INITIATED", orderId: paymentData.order_id, body: paymentData }));
        var JAZZ_CASH_MERCHANT_ID = null;
        ...
        if (paymentType === "CARD") {
            ...
            // Send the request to JazzCash
            const paymentUrl = "https://sandbox.jazzcash.com.pk/ApplicationAPI/API/Payment/DoTransaction";
            const headers = {
                "Content-Type": "application/x-www-form-urlencoded",
            };
            const response = await axios.post(paymentUrl, new URLSearchParams(sendData).toString(), { headers });
...
const initiateJazzCashPaymentAsync = async (paymentData, merchant_uid) => {
    ...
    try {
        ...
        let data = {};
        const result = await prisma.$transaction(async (tx) => {
            const merchant = await fetchMerchantAndJazzCash(tx, merchant_uid);
            ...
            const settledAmount = calculateSettledAmount(parseFloat(paymentData.amount), merchant.merchant.commissions);
            // Create Transaction
            await tx.transaction.create({
                data: {
                    ...data,
        ...
        setImmediate(async () => {
            try {
                ...
                const sendData = prepareJazzCashPayload(paymentData, jazzCashCredentials, integritySalt, formattedDate, refNo);
                if (paymentData.type.toUpperCase() === "CARD") {
                    ...
                    await processWalletPayment(sendData, refNo, result?.merchant?.merchant, phone, date, txnRefNo, result?.merchant?.jazzCashMerchant);
                }
                else {
...
const initiateJazzCashPaymentAsyncClone = async (paymentData, merchant_uid) => {
    console.log(JSON.stringify({ event: "JAZZCASH_ASYNC_INITIATED", order_id: paymentData.order_id, body: paymentData }));
    let txnRefNo;
    let refNo = "";
    const date2 = new Date();
        ...
        setImmediate(async () => {
            try {
                ...
                if (paymentData.type.toUpperCase() === "CARD") {
                    ...
                    await processWalletPaymentClone(sendData, refNo, result?.merchant?.merchant, phone, date, txnRefNo, result?.merchant?.jazzCashMerchant);
                }
                else {
                    throw new CustomError("Invalid payment type", 400);
                }
...
const fetchMerchantAndJazzCash = async (tx, merchant_uid) => {
    if (!merchant_uid) {
        throw new CustomError("Merchant UID is required", 400);
    }
...
const createTransactionReferenceNumber = (paymentData, txnDateTime, fractionalMilliseconds) => {
    if (paymentData.transaction_id) {
        return paymentData.transaction_id;
    }
    return `T${txnDateTime}${fractionalMilliseconds.toString()}${Math.random().toString(36).substr(2, 4)}`;
};
const calculateSettledAmount = (amount, commissions) => {
...
const prepareJazzCashPayload = (paymentData, credentials, integritySalt, txnDateTime, refNo) => {
    // Get the current date
    const date3 = new Date(Date.now() + (60 * 60 * 1000));
    // Define the Pakistan timezone
    const timeZone = 'Asia/Karachi';
...
const processWalletPayment = async (sendData, refNo, merchant, phone, date, txnRefNo, jazzCashMerchant) => {
    const paymentUrl = `${process.env.JAZZCASH_PAYIN_URL}/forward`;
...
const processCardPayment = async (sendData, redirectUrl) => {
    const paymentUrl = "https://sandbox.jazzcash.com.pk/ApplicationAPI/API/Purchase/PAY";
...
const jazzCashCardPayment = async (obj) => {
    let payload = {
        pp_Version: "1.1",
        pp_TxnType: "MPAY",
...
const getJazzCashMerchant = async (params) => {
    try {
        const where = {};
...
const createJazzCashMerchant = async (merchantData) => {
    try {
...
const updateJazzCashMerchant = async (merchantId, updateData) => {
    try {
...
const deleteJazzCashMerchant = async (merchantId) => {
    try {
...
const statusInquiry = async (payload, merchantId) => {
    ...
    console.log(sendData);
    sendData.pp_SecureHash = getSecureHash(sendData, merchant?.jazzCashMerchant?.integritySalt);
...
const getJazzCashInquiryChannel = async (merchantId) => {
...
const databaseStatusInquiry = async (payload, merchantId) => {
    let merchant = await prisma.merchant.findFirst({
        where: { uid: merchantId },
        include: {
            jazzCashMerchant: true,
...
const simpleStatusInquiry = async (payload, merchantId) => {
    let merchant = await prisma.merchant.findFirst({
        where: { uid: merchantId },
        include: {
            jazzCashMerchant: true,
...
const initiateJazzCashCnicPayment = async (paymentData, merchant_uid) => {
    let refNo = "";
    try {
        ...
        const time = Date.now();
        const fractionalMilliseconds = Math.floor((time - Math.floor(time)) * 1000);
        // Generate Transaction Reference Number
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (!jazzCashMerchant)
                throw new CustomError("Payment Merchant not found", 404);
            const settled_amount = parseFloat(paymentData.amount) *
                (1 -
