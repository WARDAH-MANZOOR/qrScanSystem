...
async function getMerchantCommission(merchantId, prisma) {
...
async function findOrCreateCustomer(customerName, customerEmail, merchantId, prisma) {
...
function calculateSettledAmount(originalAmount, commissionPercentage) {
    return originalAmount * (1 - +commissionPercentage);
}
async function createTransactionRecord({ order_id, id, originalAmount, type, merchantId, settledAmount, customerId, }, prisma) {
...
async function generateTransactionLink(transactionId) {
    const baseUrl = 'http://localhost:3005/payment';
    const token = createTransactionToken(transactionId);
    const transactionLink = `${baseUrl}?t=${encodeURIComponent(token)}&signupUrlResourceParams=${encodeURIComponent(transactionId)}`;
...
const createTransaction = async (obj) => {
    try {
        ...
        const transactionLink = await generateTransactionLink(result?.transaction.transaction_id);
        return {
            result,
            transactionLink
        };
...
const createTransactionService = async (body) => {
    try {
        const merchant = await prisma.merchantFinancialTerms.findUnique({
