...
describe("adjustMerchantWalletBalance", () => {
  ...
  it("should return 200 and success response when Merchant Id and target are provided", async () => {
    ...
    backofficeService.adjustMerchantWalletBalance.mockResolvedValue("Balance Adjusted");

    await backofficecontroller.adjustMerchantWalletBalance(req, res);

    // Debugging: Log actual arguments received
    console.log(
      "Called with:",
      backofficeService.adjustMerchantWalletBalance.mock.calls[0]
    );

    // Fix: Expect THREE arguments instead of two
    expect(backofficeService.adjustMerchantWalletBalance).toHaveBeenCalledWith(123, 500, true);
  ...
  it("should handle unexpected errors and return 500", async () => {
    ...
    backofficeService.adjustMerchantWalletBalance.mockRejectedValue(new Error("Unexpected Error"));
