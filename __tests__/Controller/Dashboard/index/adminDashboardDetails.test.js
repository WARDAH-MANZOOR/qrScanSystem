...
jest.mock('express-validator', () => ({
  ...
  describe("Dashboard Controller - adminDashboardDetails", () => {
    ...
    it("should return 200 and data on success", async () => {
        const mockData = { totalMerchants: 5 };
        dashboardService.adminDashboardDetails.mockResolvedValue(mockData);

        await dashboardController.adminDashboardDetails(mockReq, mockRes, mockNext);

        expect(dashboardService.adminDashboardDetails).toHaveBeenCalledWith(mockReq.query);
    ...
    it("should return 400 and error message on service failure", async () => {
        ...
        dashboardService.adminDashboardDetails.mockRejectedValue(mockError);
